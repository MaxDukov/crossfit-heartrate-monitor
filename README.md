...
**Продакшн**: Docker Compose на 192.168.0.73 — Go-бэкенд + nginx-фронтенд
(порты `:80` и `:8000`), SQLite в `./data`.

## Возможности

**Мониторинг (ТВ-экран)**
- До 8 ANT+ поясів одновременно (wildcard-каналы, автодетект новых датчиков)
...
- Watchdog: 4 подряд неуспешных ping'а БД → выход процесса (перезапуск контейнером)
